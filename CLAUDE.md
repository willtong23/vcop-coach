...
## AI 回饋機制（2026-02-28 重寫，2026-02-28 顏色系統重設計，2026-02-28 spelling/grammar 拆分）
...
### 回饋深度 Slider（Feedback Depth）
- Slider 在提交按鈕上方（提交前顯示，提交後隱藏）：「How much feedback?」
- 1-5 檔，左 Less 右 More，預設 2，不顯示程度標籤
- 影響 AI prompt 的回饋量：
  - 1-2（Light）：只給基本拼寫文法 + 最多 3 個簡單建議，多讚美，4-6 annotations
  - 3（Standard）：標準 VCOP 回饋，6-10 annotations
  - 4-5（Detailed）：深入多維度建議，具體解釋，8-14 annotations
- 值透過 `feedbackDepth` 參數傳入 `/api/analyze`

### 學生回饋維度 Toggle（6 個按鈕，全部預設關閉）
- 六個 toggle 按鈕，全部預設 OFF（學生先看乾淨原文）：
  - 📚V（Vocabulary）、🔗C（Connectives）、✨O（Openers）、🎯P（Punctuation）
  - 🔤Spelling（只控制 `type: "spelling"` 拼寫錯誤）
  - 📏Grammar（只控制 `type: "grammar"` 文法錯誤）
- Spelling 和 Grammar 各自獨立控制，互不影響
- 提交後 toggle 按鈕以 compact 模式顯示（只有 emoji，無文字標籤），與「Legend」按鈕同一行
- 圖例（FeedbackLegend）預設隱藏，點「Legend」按鈕展開
- `hiddenDimensions` state 初始值 = `new Set(["V", "C", "O", "P", "spelling", "grammar"])`
